...
class NewsDetailPage extends React.Component {
  ...
  async getNews(conHost) {
    return fetch(
      `/api/content/render/false/query/+contentType:News%20+(conhost:${conHost}%20conhost:SYSTEM_HOST)%20+languageId:1`,
      {
        headers: {
          DOTAUTH: Buffer.from(
            `${process.env.REACT_APP_USER_EMAIL}:${
  ...
  async componentDidMount() {
    ...
    // const newsList = await this.getNews(currentSite.identifier);
